...
class JJONGS_MODEL7(object):
    ...
    def train(self):
        ...
        if not(self.merge):
            ...
            train_LR_w1_file_list = sorted(glob.glob(os.path.join('merge_wild_w1', '*.npy')))
            train_LR_w2_file_list = sorted(glob.glob(os.path.join('merge_wild_w2', '*.npy')))
            train_LR_w4_file_list = sorted(glob.glob(os.path.join('merge_wild_w4', '*.npy')))
            train_LR_difficult_file_list = sorted(glob.glob(os.path.join('merge_difficult', '*.npy')))
            ...
            for ep in range(self.epoch) :   # config.epoch
                ...
                for i in range(train_range) :
                    ...
                    batch_image = np.load(train_LR_w1_file_list[i])
                    batch_image = batch_image.reshape((16,50,50,256))
                    ...
                    batch_image = np.load(train_LR_w2_file_list[i])
                    batch_image = batch_image.reshape((16, 50, 50, 256))
                    _, err = self.sess.run([self.train_op, self.loss],feed_dict={self.initial_: batch_image, self.label_images: batch_label,self.input_batch_size: self.batch_size})
                    print("Epoch : [%2d], count : [%d], time : [%4.4f], train_loss : [%.8f]" % ((ep + 1), counter, time.time() - temp_time, err))

                    batch_image = np.load(train_LR_w4_file_list[i])
                    batch_image = batch_image.reshape((16, 50, 50, 256))
                    _, err = self.sess.run([self.train_op, self.loss],feed_dict={self.initial_: batch_image, self.label_images: batch_label,self.input_batch_size: self.batch_size})
                    print("Epoch : [%2d], count : [%d], time : [%4.4f], train_loss : [%.8f]" % ((ep + 1), counter, time.time() - temp_time, err))

                    batch_image = np.load(train_LR_difficult_file_list[i])
                    batch_image = batch_image.reshape((16, 50, 50, 256))
                    _, err = self.sess.run([self.train_op, self.loss],feed_dict={self.initial_: batch_image, self.label_images: batch_label,self.input_batch_size: self.batch_size})
                    print("Epoch : [%2d], count : [%d], time : [%4.4f], train_loss : [%.8f]" % ((ep + 1), counter, time.time() - temp_time, err))
    ...
    def save(self,step) :
        model_name = "JJongs7_tensorflow.model"
        model_dir = "%s_%s" % ("JJongs7_tensorflow_scale_x", self.scale)
